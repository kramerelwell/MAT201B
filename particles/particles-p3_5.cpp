...
struct AlloApp : App {
  ...
  void reset() {
    ...
    for (int i = 0; i < 500; i++) {
      ...
      float m = 0;                                                                      // set mass
      if (i < 10) { m = planetM[i]; }                                                   
      else if (10 <= i && i < 100) { m = rnd::uniform(0.00001) + 0.00001;} 
      else if (100 <= i && i < 250) { m = rnd::uniform(0.0000077) + 0.0000077;} 
      else if (250 <= i && i < 500) { m = rnd::uniform(0.00000001) + 0.00000001;}
      mass.push_back(m);
  ...
  void onAnimate(double dt) override {
    ...
    for (int i = 0; i < mass.size(); i++){
      for (int k = i + 1; k < mass.size(); k++){
        ...
        acceleration[i] -= gravitation * ((1 - symmetry) * 2);
        acceleration[k] += gravitation * (symmetry * 2);
        }
    }
